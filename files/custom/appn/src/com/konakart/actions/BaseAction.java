...
/**
 * Base Action for KonaKart application. All of the other action classes extend
 * this class.
 */
...
// LaunchDarkly - Start
    ...
    public final static class FeatureFlagSupport {
        
        public static FeatureFlagSupport getInstance() {
            if (instance == null) {
                // Set LD_ENV_SERVER_KEY in command line when starting KonaKart
                String sdkKey = System.getenv("LD_ENV_SERVER_KEY");
                instance = new FeatureFlagSupport(sdkKey);
            }
            return instance;
        }
    
        public LDValue getJsonVariation(String flagName, String userId, LDValue defaultValue) {
            LDUser user = new LDUser(userId);
            return ldClient.jsonValueVariation(flagName, user, defaultValue);
        }
    
        private static FeatureFlagSupport instance = null;
    
        private LDClient ldClient = null;
    
        private FeatureFlagSupport(String sdkKey) {
...
// LaunchDarkly - End
     ...
     */
    /** */
    protected static final String TAG_PRODUCTS_TIME_VIEWED = "PRODUCTS_TIME_VIEWED";
    ...
    /** */
    protected static final String TAG_CATEGORIES_TIME_VIEWED = "CATEGORIES_TIME_VIEWED";

    /** */
    protected static final String TAG_MANUFACTURERS_VIEWED = "MANUFACTURERS_VIEWED";

    /** */
    protected static final String TAG_MANUFACTURERS_TIME_VIEWED = "MANUFACTURERS_TIME_VIEWED";
    ...
    /** Customer tag */
    public static final String TAG_PRODUCTS_TIME_IN_CART = "PRODUCTS_TIME_IN_CART";

    /** Customer tag */
    public static final String TAG_PRODUCTS_IN_WISHLIST = "PRODUCTS_IN_WISHLIST";

    /** Customer tag */
    public static final String TAG_PRODUCTS_TIME_IN_WISHLIST = "PRODUCTS_TIME_IN_WISHLIST";
...
// LaunchDarkly - End
     ...
     * @throws KKException    unexpected exception in KonaKart
     * @throws KKAppException unexpected exception in the application engine
     */
...
// LaunchDarkly - Start
        // LD_DELAY_FLAG_KEY needs to be defined in the command line execution of KonaKart
        String flagKey = System.getenv("LD_DELAY_FLAG_KEY");
        String delayFloorKey = "floor";
        String delayCeilingKey = "ceiling";
        LDValue defaultValue = LDValue.buildObject().put(delayFloorKey, LDValue.of(0)).put(delayCeilingKey, LDValue.of(0)).build();
        LDValue delayValue = FeatureFlagSupport.getInstance().getJsonVariation(flagKey, "demo-user@ld.se", defaultValue);

        int delayFloor = delayValue.get(delayFloorKey).intValue();
        int delayCeiling = delayValue.get(delayCeilingKey).intValue();
        int delayDuration = delayCeiling - delayFloor;
        int sleepTime = (delayDuration > 0 ? ldRandom.nextInt(delayDuration) : 0) + delayFloor;
        if(sleepTime > 0) {
            try {
...
// LaunchDarkly - End
        ...
        if (kkAppEng == null) {
            ...
            /*
             * Add KKAppEng to the struts and the HTTP sessions. In order for the struts jsp
             * tags to see it, it has to be added to the struts session as well.
             */
    ...
    /**
     * In a multi-store scenario, you should implement your own logic here to
     * extract the storeId from the request and return it.
     * 
     * @param request the HttpServletRequest request
    ...
    private String getStoreIdFromRequest(HttpServletRequest request) {
        /*
         * In multi-store mode we extract the storeId from the serverName attribute of
         * the request. In this simple implementation we look for "storeX.store." and
         * return the first part (storeX in this example) as the storeId.
         */
    ...
    protected String handleException(HttpServletRequest request, Exception e) {
        if (e != null && e.getClass().getName().equals("com.konakart.app.KKException")) {
            ...
            switch (ex.getCode()) {
                ...
                case KKException.KK_STORE_UNDER_MAINTENANCE:
                    ...
                    return new String("Unavailable");
                case KKException.KK_SESSION_EXPIRED:
                case KKException.KK_SESSION_NOT_FOUND:
                    return new String("Login");
            }
        }

        Long time = System.currentTimeMillis();
        String message = "A customer has received the following exception ( ref: " + time + " )";
        log.error(message, e);
        if (appEng != null) {
        ...
        try {
            if (appEng != null && appEng.getConfigAsBoolean(KKConstants.CONF_KEY_REPORT_STOREFRONT_EXCEPTIONS, false)) {
                // Report the problem to the System Administrator by email
                EmailOptionsIf options = new EmailOptions();
                options.setTemplateName("ExceptionNotification");
                appEng.getEng().notifySysAdmin("An unexpected problem occurred ( ref: " + time + " ) :\n\n"
                        + ExceptionUtils.exceptionToString(e), options);
            }
        } catch (KKException kke) {
            // No need to recover... or re-throw the exception - just log it.
            kke.printStackTrace();
        }
     ...
     * @throws KKAppException unexpected exception in the application engine
     */
    protected int loggedIn(HttpServletRequest request, HttpServletResponse response, KKAppEng kkAppEng,
            String forwardAfterLogin) throws KKException, KKAppException {
     ...
     */
    protected int loggedIn(HttpServletRequest request, HttpServletResponse response, KKAppEng kkAppEng,
            String forwardAfterLogin, String xsrfToken) throws KKException, KKAppException {
        return loggedIn(request, response, kkAppEng, forwardAfterLogin, /* checkXSRF */true, xsrfToken);
    }
     ...
     * @param request           the HttpServletRequest request
     * @param response          the HttpServletResponse response
     * @param checkXSRF         whether or not to check the XSRF token
     * @param xsrfToken         XSRF token
     * @return Returns the CustomerId if logged in. Otherwise a negative number.
     * @throws KKException    unexpected exception in KonaKart
     * @throws KKAppException unexpected exception in the application engine
     */
    protected int loggedIn(HttpServletRequest request, HttpServletResponse response, KKAppEng kkAppEng,
            String forwardAfterLogin, boolean checkXSRF, String xsrfToken) throws KKException, KKAppException {
        int custId;
        ...
        // Check the XSRF token for a post. Don't check anything we are redirected to
        // after a
        // login
    ...
    /**
     * If the application sends us a KKException with the cause equal to a more
     * detailed exception such as KKPasswordDoesntMatchException, sometimes the
     * application has to be able to detect this so that it can handle the exception
     * by sending an error message back to the screen.
     * 
     * @param request the HttpServletRequest request
     * @param e       The exception that has been captured
     * @param cause   The name of the cause Exception to look for i.e.
     *                KKPasswordDoesntMatchException
     * @param message The message to send out to the application
     * @param forward The forward if we detect the cause
     * @return ActionForward
     */
    protected String getForward(HttpServletRequest request, Exception e, String cause, String message, String forward) {
        if (e.getCause() != null && e.getCause().getClass().getName().equals(cause)) {
            addActionError(message);
            return forward;
        }

        // For the web services case the cause turns out to be an
        // org.apache.axis.AxisFault.
        // However within the message there is a message that says e.g. nested exception
        // is:
        // com.konakart.app.KKPasswordDoesntMatchException so we search for the name of
        // the
    ...
    /**
     * Determines whether we are using SSL or not. If we are logged in, then we
     * should be using SSL. SSL can also be forced by setting the forceSSL boolean.
     * If we should be using SSL but aren't (or vice versa) we do a redirect by
     * returning an action forward with the correct URL to redirect to. Otherwise we
     * return null
     * 
     * @param eng      a KonaKart Application engine
     * @param request  the HttpServletRequest request
    ...
    /**
     * Method used to create a browser cookie when a customer first accesses the
     * application. If the cookie already exists then we retrieve the guest customer
     * id from the cookie which will be used to retrieve and cart items that the
     * customer added to the cart on his last visit.
     * 
     * @param request  the HttpServletRequest request
     * @param response the HttpServletResponse response
     * @param kkAppEng the application engine
     * @return Returns the Customer UUID
     * @throws KKException    unexpected exception in KonaKart
     * @throws KKAppException unexpected exception in the application engine
     */
    private String manageCookies(HttpServletRequest request, HttpServletResponse response, KKAppEng kkAppEng)
        ...
        /*
         * The current customer should at this point be a guest customer with a negative
         * customer id
         */
        ...
        /*
         * Get the customerUuid from the browser cookie. A new cookie is created if it
         * doesn't exist
         */
        ...
        if (guestCustomerIdStr == null) {
             ...
             */
            setKKCookie(customerUuid, GUEST_CUSTOMER_ID, Integer.toString(currentCustomer.getId()), kkAppEng);

        } else {
            /*
             * Set the current customer id with the one retrieved from the cookie and fetch
             * any cart items that he may have.
             */
        ...
        if (log.isDebugEnabled()) {
            log.debug("GUEST_CUSTOMER_ID cookie value = " + getKKCookie(customerUuid, GUEST_CUSTOMER_ID, kkAppEng));
            log.debug("CUSTOMER_NAME cookie value = " + getKKCookie(customerUuid, CUSTOMER_NAME, kkAppEng));
            log.debug("AGREED_COOKIES cookie value = " + getKKCookie(customerUuid, AGREED_COOKIES, kkAppEng));
            log.debug("CUSTOMER_LOCALE cookie value = " + getKKCookie(customerUuid, CUSTOMER_LOCALE, kkAppEng));
            log.debug("PROD_PAGE_SIZE cookie value = " + getKKCookie(customerUuid, TAG_PROD_PAGE_SIZE, kkAppEng));
            log.debug("ORDER_PAGE_SIZE cookie value = " + getKKCookie(customerUuid, TAG_ORDER_PAGE_SIZE, kkAppEng));
            log.debug("REVIEW_PAGE_SIZE cookie value = " + getKKCookie(customerUuid, TAG_REVIEW_PAGE_SIZE, kkAppEng));
        }
    ...
    /**
     * Utility method that can be used to set a KKCookie. It attempts to get the
     * UUID from the browser cookie and creates a new browser cookie if it doesn't
     * find one.
     * 
     * @param attrId    the attrId
     * @param attrValue the attrValue
     * @param request   the HttpServletRequest request
     * @param response  the HttpServletResponse response
     * @param kkAppEng  the kkAppEng
     * @throws KKException unexpected exception in KonaKart
     */
    protected void setKKCookie(String attrId, String attrValue, HttpServletRequest request,
        ...
        /*
         * Get the CustomerUuid from the browser cookie and create the cookie if it
         * doesn't exist.
         */
     ...
     * 
     * @param customerUuid the customerUuid
     * @param attrId       the attrId
     * @param attrValue    the attrValue
     * @param kkAppEng     the kkAppEng
     * @throws KKException unexpected exception in KonaKart
     */
    protected void setKKCookie(String customerUuid, String attrId, String attrValue, KKAppEng kkAppEng)
            throws KKException {
        if (customerUuid != null) {
    ...
    /**
     * Utility method to read a KKCookie. It attempts to get the UUID from the
     ...
     * @throws KKException unexpected exception in KonaKart
     */
    protected String getKKCookie(String attrId, HttpServletRequest request, HttpServletResponse response,
     ...
     * @throws KKException unexpected exception in KonaKart
     */
    protected String getKKCookie(String customerUuid, String attrId, KKAppEng kkAppEng) throws KKException {
        if (customerUuid == null) {
            return null;
        }
    ...
    /**
     * Utility method to get the CustomerUuid from the browser cookie and create the
    ...
    private String getCustomerUuidFromBrowserCookie(HttpServletRequest request, HttpServletResponse response,
        ...
        /*
         * If the browser cookie doesn't exist then we have to create it and store a
         * newly created UUID string
         */
        if (uuid == null) {
    ...
    /**
     * When we log out, ensure that the new guest customer that is created has the
     * id saved in the browser cookie.
     * 
     * @param request  the HttpServletRequest request
     * @param response the HttpServletResponse response
     * @param kkAppEng the kkAppEng
     ...
     * @throws KKAppException unexpected exception in the application engine
     */
    protected void manageCookieLogout(HttpServletRequest request, HttpServletResponse response, KKAppEng kkAppEng)
            throws KKException, KKAppException {
        if (!kkAppEng.isKkCookieEnabled()) {
            return;
        }
        ...
        if (currentCustomer != null) {
            ...
            // Only get the basket items if we can retrieve a temporary customer from the
            // cookie
            if (guestCustomerIdStr != null) {
                try {
                    currentCustomer.setId(Integer.parseInt(guestCustomerIdStr));
    ...
    /**
     * Returns a customer event object with the action and customer id attributes
     * populated. If events aren't enabled, then null is returned.
     * 
     ...
     * @throws KKException unexpected exception in KonaKart
     */
    protected void insertCustomerEvent(KKAppEng kkAppEng, int action, String str1, String str2, int int1, int int2,
            BigDecimal dec1, BigDecimal dec2) throws KKException {
    ...
    /**
     * Shortcut method for inserting a customer event passing an integer as event
     * data
     * 
     * @param kkAppEng the kkAppEng
     * @param action   the action
     ...
     * @throws KKException unexpected exception in KonaKart
     */
    protected void insertCustomerEvent(KKAppEng kkAppEng, int action, String str1) throws KKException {
        insertCustomerEvent(kkAppEng, action, str1, null, 0, 0, null, null);
    }

    /**
     * Shortcut method for inserting a customer event passing a decimal as event
     * data
     * 
     * @param kkAppEng the kkAppEng
     * @param action   the action
     ...
     * @throws KKException unexpected exception in KonaKart
     */
    protected void insertCustomerEvent(KKAppEng kkAppEng, int action, BigDecimal dec1) throws KKException {
        insertCustomerEvent(kkAppEng, action, null, null, 0, 0, dec1, null);
    }

    /**
     * Method called when a customer logs in or logs out. When logging in we need to
     * decide whether to update the customer's PRODUCTS_TIME_VIEWED tag value from
     * the value of the guest customer's tag. When logging out we need to make the
     * same decision in the opposite direction. We only do the updates if the tag
     * value of the "oldTag" is more recent than the tag value of the "newTag".
     * 
     * @param kkAppEng the KonaKart Application engine
     * @param oldTag   When logging in, it is the tag of the guest customer. When
     *                 logging out, it is the tag of the logged in customer.
     * @param newTag   When logging in, it is the tag of the logged in customer.
     *                 When logging out, it is the tag of the guest customer.
     * @throws KKException    unexpected exception in KonaKart
     * @throws KKAppException unexpected exception in the application engine
     */
    protected void updateRecentlyViewedProducts(KKAppEng kkAppEng, CustomerTagIf oldTag, CustomerTagIf newTag)
            throws KKAppException, KKException {
        if (oldTag != null && oldTag.getDateAdded() != null && oldTag.getValue() != null
            ...
            if (newTag == null || newTag.getDateAdded() == null
                ...
                /*
                 * If new tag doesn't exist or old tag is newer than new tag, then give newTag
                 * the value of old tag
                 */
                kkAppEng.getCustomerTagMgr().insertCustomerTag(TAG_PRODUCTS_TIME_VIEWED, oldTag.getValue());
            }
        }
    }

    /**
     * Method changes the session before performing the login in order to avoid a
     ...
     * @throws KKAppException unexpected exception in the application engine
     */
    protected LoginResultIf login(KKAppEng kkAppEng, HttpServletRequest request, String loginUsername, String password)
            throws KKException, KKAppException {
        if (!kkAppEng.isPortlet()) {
            // Change the session
            changeSession(request);
    ...
    /**
     * Creates a new session when we switch to SSL to avoid hackers using the
     * current session id (which may have been visible on the URL) to log into the
     * application
     * 
     * @param request the HttpServletRequest request
     */
    protected void changeSession(HttpServletRequest request) {
        try {
            ...
            /*
             * Loop through all objects saved in the current session and place them in the
             * hash map
             */
            ...
            log.warn("Exception occurred while trying to change session for security reasons typically after a login",
                    e);
        }
    }

    /**
     * Common code for setting up the redirect response. A few headers are set to
     * stop the browser from caching the permanent redirect.
     * 
     * @param response the HttpServletResponse response
     */
    ...
    /**
     * Common code for setting up the redirect response when switching from HTTP to
     * HTTPS and vice versa
     * 
     * @param response     the HttpServletResponse response
     * @param redirForward the redirect forward to set in the Location parameter in
    ...
    /**
     * A URL encoder which converts spaces to "-" and doesn't encode forward or back
     * slashes since many app servers don't accept these because of security issues.
     * 
     * @param in the in
     * @return Returns an encoded string
     * @throws UnsupportedEncodingException an unexpected
    ...
    /**
     * Utility method called from JSPs to return an empty string when the value of
     ...
     * 
     * @param request    the HttpServletRequest request
     * @param addContext If true the result looks like
     *                   http://localhost:8780/konakart otherwise it's like
     *                   http://localhost:8780
     * 
    ...
    /**
     * @param jspEng the jspEng to set
     */
    ...
    /**
     * Common method used to encode certain characters. The default is now to use
     * the Guava Escaper that only escapes the following five dangerous ASCII
     * characters {@code '"&<>} since most people use UTF-8 encoding. If you want to
     * encode many more HTML characters you can use the Apache StringEscapeUtils as
     * follows
     * <p>
